...
class Board:
    def __init__(self,width,height, p1_sprites, p2_sprites):
        ...
        self.turn = 0
        self.power_ups = {'Shield': 'S', 'Speed Boost': 'B'}  # Power-up symbols
        self.generate_power_ups()  # Generate initial power-ups

    def generate_power_ups(self):
        """Generate power-ups randomly on the board."""
        available_cells = [(i, j) for i in range(self.height) for j in range(self.width) if self.board[i][j] == 0]
        num_power_ups = min(len(available_cells), 2)  # Maximum 2 power-ups on the board
        power_ups_to_place = random.sample(list(self.power_ups.values()), num_power_ups)
        for symbol in power_ups_to_place:
            row, col = random.choice(available_cells)
            self.board[row][col] = symbol

    def apply_power_up_effect(self, row, col, player):
        """Apply the effect of a power-up to the player."""
        power_up = self.board[row][col]
        if power_up == 'S':
            # Apply Shield power-up: immunity to overflow for 2 turns
            self.turn += 2
        elif power_up == 'B':
            # Apply Speed Boost power-up: grant an extra turn
            self.turn -= 1
        # Remove the power-up from the board
        self.board[row][col] = 0

    # Add Bomb piece logic
    def place_bomb(self, row, col, player):
        """Place a Bomb piece on the board and clear surrounding pieces."""
        if self.valid_move(row, col, player):
            self.board[row][col] = 'X'  # Bomb piece symbol
            # Clear surrounding pieces
            for i in range(row - 1, row + 2):
                for j in range(col - 1, col + 2):
                    if 0 <= i < self.height and 0 <= j < self.width and self.board[i][j] != 'X':
                        self.board[i][j] = 0
            self.turn += 1
            return True
        return False
    ...
    def valid_move(self, row, col, player):
        if row >= 0 and row < self.height and col >= 0 and col < self.width and (self.board[row][col] == 0 or self.board[row][col] == player):
            return True
        return False
    ...
    def draw(self, window, frame):
        ...
        for row in range(self.height):
             for col in range(self.width):
                ...
                if self.board[row][col] == 'X':  # Check for Bomb piece
                    # Draw Bomb piece using a different color or symbol
                    pygame.draw.circle(window, (cpos + CELL_SIZE // 2, rpos + CELL_SIZE // 2), CELL_SIZE // 4)
                elif isinstance(self.board[row][col], int) and self.board[row][col] > 0:
                    sprite = p1_sprites
                elif isinstance(self.board[row][col], int) and self.board[row][col] < 0:
                    sprite = p2_sprites
                else:
                    continue  # Skip if it's neither a player piece nor a Bomb piece
